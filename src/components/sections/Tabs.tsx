...
const TABS_IDS = ["reliability", "transparency", "scale", "growth"];

export function Tabs() {
    const t = useTranslations('Tabs');
    const [activeTab, setActiveTab] = useState(TABS_IDS[0]);
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 xl:gap-24 items-start">
                    ...
                    <div className="lg:col-span-5 space-y-16">
                        <div className="space-y-6">
                            ...
                            >
                                {t('heading1')} <span className="text-foreground/40">{t('headingHighlight')}</span>
                            </motion.h2>
                        </div>
                            ...
                            {TABS_IDS.map((tabId, index) => (
                                <button
                                    ...
                                    className={cn(
                                        ...
                                        activeTab === tabId ? "text-foreground" : "text-foreground/30 hover:text-foreground/60"
                                    )}
                                >
                                        ...
                                        {[0, 1, 2].map((idx) => (
                                            <div key={idx} className="flex items-start gap-4 group/item">
                                                ...
                                                <span className="text-sm md:text-base text-foreground/70 font-medium tracking-tight group-hover/item:text-foreground transition-colors duration-500">
                                                    {t(`items.${activeTab}.bullets.${idx}` as any)}
                                                </span>
                                            </div>
                                        ))}
                                    </div>
