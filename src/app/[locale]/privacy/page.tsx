...
export const metadata: Metadata = {
    title: "Privacy Policy | PRISM IT Studio",
    description: "Privacy Policy for PRISM IT Studio — how we collect, use, and protect your information.",
};
            ...
            <Container>
                <div className="pt-44 pb-32 max-w-3xl mx-auto">
                    ...
                    <div className="space-y-12 text-foreground/70 leading-relaxed">
                        {sections.map((num) => {
                            const sectionList = t.raw(`privacySections.${num}.list` as any) as string[] | undefined;
                            return (
                                <div key={num}>
                                    <section className="space-y-4">
                                        <h2 className="text-xl font-heading font-medium text-foreground tracking-tight">{t(`privacySections.${num}.title` as any)}</h2>
                                        <p>{t(`privacySections.${num}.p` as any)}</p>
                                        ...
                                        {sectionList && sectionList.length > 0 && (
                                            <ul className="list-disc list-inside space-y-2 ml-4 text-foreground/60">
                                                {sectionList.map((item, idx) => (
                                                    <li key={idx} dangerouslySetInnerHTML={{ __html: item.replace(/^(.*?):/, '<strong class="text-foreground/80">$1:</strong>') }} />
                                                ))}
                                            </ul>
                                        )}
                                        
                                        {num === 7 && (
                                            <p>To exercise these rights, contact us at <a href="mailto:xurshidbahromov06@gmail.com" className="text-blue-500 hover:underline">xurshidbahromov06@gmail.com</a></p>
                                        )}
